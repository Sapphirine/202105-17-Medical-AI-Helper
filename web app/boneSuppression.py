...
def bone_suppresion(Img_path):
    ...
    parser.add_argument("--desired_l1_loss", default=0.005,
                        help="stop parameter: desired l1 loss for early stop the training < max_epochs ")
    parser.add_argument("--patience_epochs", default=100,
                        help="stop parameter: number of epochs in which the loss of generator increased while the loss of discriminator decreased.")
    ...
    parser.add_argument("--kernelsize", type=int, default=4, help="kernelsize for the conv layers")
    # export options
    ...
    def discrim_conv(batch_input, out_channels, stride):
        ...
        return tf.layers.conv2d(padded_input, out_channels, kernel_size=a.kernelsize, strides=(stride, stride),
    ...
    def gen_conv(batch_input, out_channels):
        ...
        if a.separable_conv:
            return tf.layers.separable_conv2d(batch_input, out_channels, kernel_size=a.kernelsize, strides=(2, 2),
                                              padding="same", depthwise_initializer=initializer,
                                              pointwise_initializer=initializer)
        else:
            return tf.layers.conv2d(batch_input, out_channels, kernel_size=a.kernelsize, strides=(2, 2), padding="same",
    ...
    def gen_deconv(batch_input, out_channels):
        ...
        if a.separable_conv:
            ...
            return tf.layers.separable_conv2d(resized_input, out_channels, kernel_size=a.kernelsize, strides=(1, 1),
                                              padding="same", depthwise_initializer=initializer,
                                              pointwise_initializer=initializer)
        else:
            return tf.layers.conv2d_transpose(batch_input, out_channels, kernel_size=a.kernelsize, strides=(2, 2),
    ...
    def CombineImages(ImageListNames, task_No, input_dir_all):
        ...
        for filename in ImageListNames:
            ...
            Image1 = io.imread(a.input_dir_all + '/Inputs/' + filename + '.png', as_gray=False)
            Size = Image1.shape
            if len(Size) == 2:
                Image_ = np.zeros(shape=[Image1.shape[0], Image1.shape[1], 3], dtype='uint8')
                Image_[:, :, 0] = Image1
                Image_[:, :, 1] = Image1
                Image_[:, :, 2] = Image1
                Image1 = Image_
            ...
            if task_No == 2:
                Image_t2 = io.imread(a.input_dir_all + '/Targets_' + str(task_No) + '/' + filename + '.png',
                                     as_gray=False)
                Size = Image_t2.shape
                if len(Size) == 2:
                    Image1_ = np.zeros(shape=[Image_t2.shape[0], Image_t2.shape[1], 3], dtype='uint8')
                    Image1_[:, :, 0] = Image_t2
                    Image1_[:, :, 1] = Image_t2
                    Image1_[:, :, 2] = Image_t2
                    Image_t2 = Image1_
                Image_combined = np.zeros(shape=[Image1.shape[0], 2 * Image1.shape[1], 3], dtype='uint8')
                Image_combined[:, :Image1.shape[1], :] = Image1
                Image_combined[:, Image1.shape[1]:, :] = Image_t2
                Image_combined = tf.image.resize_images(Image_combined, size=[512, 1024],
                                                        method=tf.image.ResizeMethod.BICUBIC)
                Image_combined = tf.Session().run(Image_combined)
                # Image_combined=tf.image.resize_images(Image_combined, size=[512,1024], method=tf.image.ResizeMethod.BICUBIC)
                # Image_combined=tf.Session().run(Image_combined)

                io.imsave(write_to_dir + filename + '.png', Image_combined)

    a.input_dir_all = Img_path
    a.cv_info_dir = 'C:\Chaoran\6895\app\webApp1\webApp1\static\BoneSuppression\cv_info'
    a.output_dir_all = 'C:\Chaoran\6895\app\webApp1\webApp1\static\BoneSuppression\outputs'
    a.task_No = 2
    a.max_epochs = 2000
    a.desired_l1_loss = 0.02
    a.batch_size = 1
    a.seed = 35555
    a.lr = 1e-7
    a.ngf = 64
    a.kernelsize = 3
    a.patience_epochs = 20
    ...
    for cv in range(0, len(CvDirs)):  # #################
        ...
        # if(a.mode=='train'):
        # print('############',a.mode)
        # a.checkpoint=a.output_dir_all+'/pix2pix_orig_ST/Models_t'+str(a.task_No)+'/'+SetNames[cv]
        ...
        # a.output_dir=a.output_dir_all+'/pix2pix_orig_ST/Models_t'+str(a.task_No)+'/'+SetNames[cv]
        # main()
        a.mode = 'test'
        if (a.mode == 'test'):
            print('############', a.mode)
            ...
            a.checkpoint = 'C:\Chaoran\6895\app\webApp1\webApp1\static\BoneSuppression\model\set_1\checkpoint'
            a.input_dir = write_to_dir
            a.output_dir = 'C:\Chaoran\6895\app\webApp1\webApp1\static\BoneSuppression\outputs'
            # try:
            ...
            # except:
            # no=1
    return a.output_dir_all

UPLOAD_FOLDER1 = 'static\\BoneSuppression\\inputs'
...
@app.route("/Bonesupquery", methods=["POST"])
def Bonesupquery():

    if request.method == 'POST':
        # RECIBIR DATA DEL POST
        if 'file' not in request.files:
            return render_template('bone.html',filename='no image')
        ...
        if file and allowed_file(file.filename):
            ...
            img_path = "static\\BoneSuppression\\inputs\\" + filename
            #with open('C:\Chaoran\6895\app\webApp1\webApp1\static\BoneSuppression\cv_info', 'w') as f:
                #text = filename
                #f.write(text)
            file.save(img_path) #将图存到了CT_img文件夹里面
            image_name = filename
            #path = bone_suppression(img_path)
            path = 'static\\BoneSuppression\\outputs\\' + filename
            #输出图到HTML上
            return render_template('bone.html', ori_lung_image= img_path, bone_sup_image=path)
