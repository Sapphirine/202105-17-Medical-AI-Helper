# based on https://github.com/mohaEs/image-to-images-translation
...
def CombineImages(ImageListNames, task_No, input_dir_all):
    ...
    for filename in ImageListNames:
        ...
        if task_No==2: 
            Image_t2=io.imread(a.input_dir_all+'/Targets_'+str(task_No) +'/'+ filename+'.png',as_gray=False)
            Size=Image_t2.shape
            if len(Size)==2:
...
for cv in range(0,len(CvDirs)): # #################
    ...
    if(a.mode=='test'):
        ...
        a.output_dir=a.output_dir_all+'/pix2pix_orig_ST/Results_t'+str(a.task_No)+'/'+SetNames[cv]
